...
-------------------------------------------------------------------------------
 ...
 Rev:    $Revision: 250 $
 URL:    $HeadURL: svn://svn.icculus.org/cdogs-sdl/trunk/src/input.c $
 ID:     $Id: input.c 250 2007-07-06 16:38:43Z lmartinking $
	...
	if (cmd) {
		if (data->controls == JOYSTICK_ONE) {
			...
			*cmd = 0;
			...
			if ((pad.Buttons & PSP_CTRL_LEFT) || (pad.Lx < 10))
				*cmd |= CMD_LEFT;
			else if ((pad.Buttons & PSP_CTRL_RIGHT) || (pad.Lx > 245))
				*cmd |= CMD_RIGHT;
			if ((pad.Buttons & PSP_CTRL_UP) || (pad.Ly < 10))
				*cmd |= CMD_UP;
			else if ((pad.Buttons & PSP_CTRL_DOWN) || (pad.Ly > 245))
				*cmd |= CMD_DOWN;
